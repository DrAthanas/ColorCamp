...
class Scale(tuple):
    ...
    def get_color(self, value, min_val:Union[float, int]=0, max_val:Union[float, int]=1):
        # TODO: Vectorize this
        ...
        relative_value = (norm_value - self.stops[max_idx-1]) / (self.stops[max_idx] - self.stops[max_idx-1]) 
    ...
    def _repr_html_(self):
        grad = ', '.join([f"{color.hex} {stop:.0%}" for color, stop in zip(self, self.stops)])
        html_string = DIV_TEMPLATE.format(grad = grad, height = 60, width = max(60*self.len, 180))
